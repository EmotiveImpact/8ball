...
def applicable_constraints(case: Case, action: Action, states: dict):
    hard,soft=[],[]
...
def readiness(case: Case, action: Action, states: dict, now: datetime) -> dict:
...
def changes(before:Case,after:Case,bp:dict,ap:dict) -> dict:
    ...
    metadata=[name for name in ('title','desired_outcome','status','priority','owner','next_update') if getattr(before,name)!=getattr(after,name)]
