...
class Store(LegacyStore):
    ...
    def _persist(self,db,before,after,command,actor,now):
        ...
        self._append(db,after,command.event_id,command.kind,command.payload,actor,digest(command.model_dump(mode='json')),delta)
        return delta
    ...
    def audit(self,case_id):
        ...
        return {'format':'eightball-case-v2','valid':valid,'head_hash':previous,'case':case.model_dump(mode='json'),
