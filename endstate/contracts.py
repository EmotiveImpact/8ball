...
from .primitives import Strict, Identifier, Title, Text, Evidence, Observation, uid, utcnow
...
class PlanningSnapshot(Strict):
    """Domain-neutral, read-only-by-convention input; authoritative state belongs to the caller.

    The public calculate() boundary makes and validates its own copy. No model,
    database, user account, case summary or fixer-specific playbook is required.
    """
    id: Identifier = Field(default_factory=uid)
    owner: Title = 'Operator'
...
def validate_snapshot_relations(self):
